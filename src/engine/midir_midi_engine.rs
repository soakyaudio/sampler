use crate::base::{MidiReceiver, MidiEngine, MidiMessage};
...
impl MidirMidiEngine {
    ...
    pub fn new(receiver: impl MidiReceiver + Clone + 'static) -> Self {
        ...
        let scanner = midir::MidiInput::new("sampler_scanner")
            .expect("Failed to create MIDI input client.");
        ...
        for (i, port) in ports.iter().enumerate() {
            ...
            let midi_input = midir::MidiInput::new(&port_name)
                .expect("Failed to create MIDI input client.");
            let conn = midi_input.connect(port, &port_name, move |_timestamp, bytes, _data| {
